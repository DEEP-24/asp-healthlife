    ...
    <>
      <div className="m-2 rounded bg-red-100 p-4">
        ...
        <p className="mb-1 text-lg">
          An error has occurred processing your request. You may try again or contact support if the
          problem persists.
        </p>
      </div>
      {stack && (
...
function Unauthorized({ message }: { data: any; message: string }) {
  return (
    <div className="m-2 rounded bg-purple-100 p-4">
      ...
      <p className="mb-1 text-lg">
        You must be logged into access this page. Click <Link to="/login">here</Link> to login.
      </p>
    </div>
  );
}
...
function Forbidden({ message }: { data: any; message: string }) {
  return (
    <div className="m-2 rounded bg-orange-100 p-4">
      ...
      <p className="mb-1 text-lg">You are not authorized to access this page.</p>
    </div>
  );
}
...
function NotFound({ message }: { data: any; message: string }) {
  return (
    <div className="m-2 rounded bg-blue-100 p-4">
      ...
      <p className="mb-1 text-lg">The page you were looking for could not be found.</p>
    </div>
  );
}
